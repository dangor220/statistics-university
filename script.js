let str = prompt('Введите значения совокупностей через запятую: ');
...
if (str !== null && str !== '') {
  ...
  function mediana(arr) {
    ...
    return (
      (arr[Math.round(mediumMediand)] + arr[Math.round(mediumMediand) - 1]) / 2
    );
  }
...
let strTwo = prompt('Введите значения выборки совокупностей через запятую');


if (strTwo !== null && strTwo !== '') {
  let arrMap = strTwo.split(',');
  let arr = arrMap.map(Number).sort((a, b) => a - b);
  ...
  function disp(arr, medium) {
    ...
    return parseFloat(Math.sqrt(sumArr / (num - 1)).toFixed(1));
  }
  ...
  // console.log(gaus(disp(arr, medium(arr)), medium(arr), arr));
  console.log(arr);
  console.log('Выборочное среднее: ', medium(arr));
  console.log('Выборочное стандартное отклонение: ', disp(arr, medium(arr)));
  console.log('Медиана: ', mediana(arr));
  console.log('75-процентиль: ', sevenfiveProc(arr));
  console.log('25-процентиль: ', twofiveProc(arr));
}
